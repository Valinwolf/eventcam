	...
	var body: some View {
		NavigationStack {
			List {
				...
				Section {
					Text("Clearing history does not delete uploaded or already-shared event media.")
						.font(.footnote)
						.foregroundStyle(.secondary)
				}
			...
			.alert("Delete History Item", isPresented: Binding(
				...
				Text("This removes the selected event code from local history only. It does not delete uploaded or shared event media.")
			}
			.alert("Clear All History", isPresented: $showClearAllConfirmation) {
				...
				Text("This removes all saved event codes from local history only. It does not delete uploaded or shared event media.")
			}
		}
	}
